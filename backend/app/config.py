...
class Settings(BaseSettings):
    ...
    openai_api_key: str = Field(..., env="OPENAI_API_KEY")  # type: ignore[call-arg]
    # Basic auth token for protecting the chat endpoint.
    # Send `Authorization: Bearer <token>`.
    auth_token: Optional[str] = Field(None, env="JULES_AUTH_TOKEN")  # type: ignore[call-arg]
    # Path to the persistent LangGraph checkpoint database.
    checkpoint_db: str = Field("data/checkpoints.sqlite", env="JULES_CHECKPOINT_DB")  # type: ignore[call-arg]
    ...
    SEARCH_TOP_K: conint(ge=1) = Field(8, env="SEARCH_TOP_K")  # type: ignore[call-arg]
    # Oversampling factor – we first fetch TOP_K * oversample candidates for MMR.
    SEARCH_MMR_OVERSAMPLE: conint(ge=1) = Field(4, env="SEARCH_MMR_OVERSAMPLE")  # type: ignore[call-arg]
    # λ ∈ [0,1] – 0→novelty-only, 1→relevance-only.
    SEARCH_MMR_LAMBDA: confloat(ge=0.0, le=1.0) = Field(0.5, env="SEARCH_MMR_LAMBDA")  # type: ignore[call-arg]
